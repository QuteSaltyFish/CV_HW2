...
def printcoords():
    ...
    try:
        ...
        pic = hw2(File, DEVICE)
        condition_dir = filedialog.askopenfilename(title='Choose an condition.')
        condition_pic = Image.open(condition_dir).convert("L")
        condition_pic.resize([pic.img_gray_tensor.shape[1], pic.img_gray_tensor.shape[2]])
        condition = transforms.ToTensor()(condition_pic).to(DEVICE)
        condition = pic.to2(data=condition)
        pic.Conditional_Dilate(condition, SE/255)
        # show the picture
        filename = ImageTk.PhotoImage(Image.open('result/Conditional_Dilate.gif'))
        ...
        l3.config(text='Pls input H and W')
